...
<template>
  <div class="w-full">
    <ChatMessages v-if="messages.length > 0" :messages
      class="p-4 border-2 border-solid border-gray-200 rounded-t-lg bg-white" />
    <ChatInput class="p-4 bg-gray-200 rounded-b-lg" :class="{ 'rounded-lg': messages.length === 0 }"
      @submit="handleSubmit" />
  </div>
</template>